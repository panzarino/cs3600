...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            transitionModel = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            for posTrans in transitionModel:
                beliefs[posTrans] += transitionModel[posTrans] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            for part in self.particles:
                beliefs[part] += emissionModel[util.manhattanDistance(part, pacmanPosition)]
            self.beliefs = beliefs
        else:
        ...
        if self.beliefs.totalCount() != 0:
            ...
            for x in range(len(self.particles)):
                self.particles[x] = util.sample(self.beliefs)
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        """
        for x in range(len(self.particles)):
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[x]))
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for x in range(self.numGhosts):
            if noisyDistances[x] == None:
                for y in range(self.numParticles):
                    self.particles[y] = self.getParticleWithGhostInJail(self.particles[y], x)
        weighted = util.Counter()
        for part in self.particles:
            weight = 1
            for x in range(self.numGhosts):
                if noisyDistances[x] != None:
                    weight *= emissionModels[x][util.manhattanDistance(part[x], pacmanPosition)]
            weighted[part] += weight
        if weighted.totalCount() == 0:
            self.initializeParticles()
        else:
            self.particles = [util.sample(weighted) for x in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for x in range(self.numGhosts):
                newParticle[x] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)), x, self.ghostAgents[x]))
