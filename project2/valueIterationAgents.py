...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for i in range(self.iterations):
            ...
            for state in states:
                valActions = util.Counter()
                for action in self.mdp.getPossibleActions(state):
                    valActions[action] = self.computeQValueFromValues(state, action)
                valState[state] = valActions[valActions.argMax()]
            for state in states:
                self.values[state] = valState[state]
